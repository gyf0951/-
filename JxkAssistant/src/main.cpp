...
//唤醒词配置
#define RX_PIN 10  // ASRPRO的TX连接到ESP32S3的RX (GPIO10)
#define TX_PIN 11  // ASRPRO的RX连接到ESP32S3的TX (GPIO11)
...
String wakeup_word="Detected";
String wakeup_anwser="我在呢";
static bool record_status = true; //录音状态  true:监听中  false:表示进入录音模式 
...
void setup() {
  Serial.begin(115200);
  mySerial.begin(115200, SERIAL_8N1, RX_PIN, TX_PIN); // 初始化UART1（ASRPRO连接串口）
...
void loop() {
  String recvData=WakeupDetect();
  if(recvData.indexOf(wakeup_word)!=-1){
    record_status=false;
    Serial.println("唤醒词检测成功");
    baiduTTS_Send(access_token, wakeup_anwser);
    delay(1000);
  }
...
String WakeupDetect(){
  ...
  if (mySerial.available()) {
    ...
    data = mySerial.readString();  // 读取字符串
